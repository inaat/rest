...
<section class="main-content-wrapper">
    ...
    <div class="box-wrapper">
        ...
        <div class="table-box">
            ...
            <?php
            $attributes = array('id' => 'add_role');
            echo form_open_multipart(base_url('Role/addEditRole/' . $encrypted_id."/".$copy), $attributes); ?>
            <div class="box-body">
                <div class="row">
                    <div class="col-sm-12 mb-2 col-md-3">
                        <div class="form-group">
                            <label><?php echo lang('role_name'); ?> <span class="required_star">*</span></label>
                            <input tabindex="1" autocomplete="off" type="text" name="role_name" class="form-control" placeholder="<?php echo lang('name'); ?>" value="<?php echo escape_output($role_details->role_name); ?>">
                        </div>
                            ...
                            foreach($value->functions as $keys_f=>$value_f) {
                                ...
                                <div class="">
                                    <label class="font_unset container txt-uh-77"><?php echo escape_output(lang($value_f->label_name)); ?>
                                        <input type="checkbox" <?php echo escape_output($checked)?> class="checkbox_role child_class child_<?php echo str_replace(' ', '_', $value->module_name)?>" data-parent_name="<?php echo str_replace(' ', '_', $value->module_name)?>" value="<?php echo escape_output($value->id); ?>|<?php echo escape_output($value_f->id); ?>"  name="access_id[]">
                                        <span class="checkmark"></span>
                                    </label>
                                </div>
                                <?php
