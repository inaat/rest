...
<section class="main-content-wrapper">
    ...
    <section class="content-header">
        <h3 class="top-left-header">
            <?php echo lang('order_receiving_user'); ?> (<?php echo lang('online_order'); ?>)
        </h3>
    ...
    <div class="box-wrapper">
        <div class="table-box">
            <?php echo form_open(base_url() . 'setting/orderReceivingUserOnlineOrder/', $arrayName = array('id' => 'update_tax_setting','enctype'=>'multipart/form-data')) ?>
            <div class="box-body">
                <div class="row">
                    <?php 
                        ...
                        <div class="col-sm-12 mb-3 col-md-4 col-lg-3">
                            <div class="form-group">
                                <label> <?php echo escape_output($key)?>. <?php echo lang('outlet'); ?>: <b><?php echo escape_output($outlet->outlet_name)?></b></label>
                                          <select class="form-control select2 ir_w_100" name="online_order_receiving_id[]" id="online_order_receiving_id">
                                          <option value=""><?php echo lang('receiving_user'); ?></option>
                                          <?php
                                          foreach ($waiters as $value):
                                                      ?>
                                                      <option  <?php echo ($outlet->online_order_receiving_id==$value->id)?'selected':''?> value="<?=$value->id?>"><?=$value->full_name?> (<?=$value->designation?>)</option>
                                                      <?php 
                                          endforeach;
                                          ?>
                                    </select>
            ...
            <div class="box-footer">
                ...
                <a class="btn bg-blue-btn" href="<?php echo base_url() ?>setting/onlineOrderEnableDisable">
